...
import { ConfirmDialog, PrinterSettingsDialog } from "../../component";
...
export const TableOrdersDialog = ({
    ...
    useEffect(() => {
        const checkAgent = async () => {
            try {
                ...
                setPrintAgentReady(!!json?.ok);
            } catch {
                setPrintAgentReady(false);
            }
        };
        checkAgent();
    ...
    useEffect(() => {
        const totalItems = ordersList.flatMap((o) => o.itemsArr).filter((it) => it.cartId).length;
        const selectedCount = Object.keys(selected).length;
        setSelectAll(totalItems > 0 && selectedCount === totalItems);
    }, [selected, ordersList]);

    const openConfirm = (
        next: Omit<Extract<ConfirmState, { open: true }>, "open">
    ) => {
    ...
    const handleDeleteCartItem = async (orderId: string, cartId: string) => {
        ...
        setSelected((prev) =>
            applyLocalDeleteCartItemSelection(orderId, cartId, prev)
        );
    };
    ...
    const handleSelectAll = () => {
        if (selectAll) {
            ...
            ordersList.forEach((o) => {
                o.itemsArr.forEach((it) => {
                    if (it.cartId) {
                        next[selKeyToString({ kind: "item", orderId: o.orderId, cartId: it.cartId })] = true;
                    }
                });
            });
            setSelected(next);
    ...
    const handleDeleteSelected = async () => {
        for (const o of ordersList) {
            const allItemsSelected = o.itemsArr.every(
                (it) => it.cartId && isSelected({ kind: "item", orderId: o.orderId, cartId: it.cartId })
            );
            if (allItemsSelected || isSelected({ kind: "order", orderId: o.orderId })) {
                await handleDeleteOrderNode(o.orderId);
            } else {
                for (const it of o.itemsArr) {
                    if (it.cartId && isSelected({ kind: "item", orderId: o.orderId, cartId: it.cartId })) {
                        await handleDeleteCartItem(o.orderId, it.cartId);
                    }
                }
            }
        }
    ...
    const rightTotal = useMemo(
        () => computeRightTotal(selectedItemsForRight),
        [selectedItemsForRight]
    );
                ...
                <DialogTitle sx={{ p: 0 }}>
                    ...
                    >
                        ...
                        <Stack direction="row" alignItems="center" spacing={2}>
                            ...
                            <Chip
                                label={`Genel Toplam: ${grandTotal} TL`}
                                color="default"
                                sx={{ fontWeight: 800 }}
                            />
                        </Stack>
                ...
                <DialogContent dividers sx={{ p: isMobile ? 1 : 2 }}>
                    {!printAgentReady && (
                        <Alert severity="warning" sx={{ mb: 2 }}>
                            Print Agent çalışmıyor. Lütfen yazdırma servisini başlatın.
                        </Alert>
                    )}
                                        ...
                                        <FormControlLabel
                                            ...
                                            control={
                                                <Checkbox
                                                    size="small"
                                                    checked={selectAll}
                                                    indeterminate={!selectAll && Object.keys(selected).length > 0}
                                        ...
                                        <Tooltip title="Seçilenleri sil">
                                            <span>
                                                <IconButton
                                                    color="error"
                                                    size="small"
                                                    disabled={Object.keys(selected).length === 0 || (!!user && !user.isAdmin)}
                                                    onClick={() =>
                                                        openConfirm({
                                                            title: "Seçilen ürünleri sil?",
                                                            description: `${Object.keys(selected).length} seçim silinecek.`,
                                                            confirmLabel: "Sil",
                                                        ...
                                                        <IconButton
                                                            ...
                                                            onClick={() =>
                                                                openConfirm({
                                                                    ...
                                                                    confirmLabel: "Sil",
                                                                    action: () =>
                                                                        handleDeleteOrderNode(o.orderId),
                                                                })
                                                            }
                                                            sx={{ color: "error.main" }}
                                                    ...
                                                    <FormControlLabel
                                                        ...
                                                        control={
                                                            <Checkbox
                                                                ...
                                                                onChange={() =>
                                                                    toggleAll(o.orderId, o.itemsArr)
                                                                }
                                                            />
                                                        }
                                                        label={
                                        ...
                                        {selectedItemsForRight.length === 0 ? (
                                            ...
                                            <Stack spacing={1}>
                                                {selectedItemsForRight.map((x, i) => {
                                                    ...
                                                    const unit =
                                                        typeof x.item.unitPrice === "number"
                                                            ? x.item.unitPrice
                                                            : 0;
                                                    const lineTotal = unit * qty;
